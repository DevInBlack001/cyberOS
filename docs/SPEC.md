...
`cyberos-store` — a CLI plus a GTK4/libadwaita GUI, reusing the stack the installer wizard
already pulls in (`python-gobject`, `gtk4`, `libadwaita` are already in
`packages.x86_64`). It reads an AppStream catalogue generated by `appstreamcli compose`
and published beside the repo database, and installs via `pkexec pacman`.
